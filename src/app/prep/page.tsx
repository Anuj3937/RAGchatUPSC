...
import type { ChatMessage, QuestionForEvaluation, MCQQuestion, SubjectiveQuestion } from "@/lib/types";

type TestType = "mcq" | "subjective";
type ActionType = "summary" | "mcq" | "subjective" | "flashcards";
...
export default function PrepPage() {
  ...
  const addMessage = (role: 'user' | 'assistant', content: React.ReactNode) => {
    setChatHistory(prev => [...prev, { id: `${Date.now()}-${nextId.current++}`, role, content }]);
  };
  ...
  const executeAction = async (action: ActionType, topic: string, document?: FileState) => {
    ...
    const currentTopic = topic || document?.name || lastTopic || "the provided document";
    if (!currentTopic && action !== 'summary') {
      toast({ variant: "destructive", title: "No Topic", description: "Please provide a topic or upload a document." });
      setIsLoading(false);
      return;
    }
    ...
    try {
      if (action === "summary") {
        if (!document) {
          addMessage('assistant', "Please upload a document to get a summary.");
          return;
        }
        addMessage('user', `Summarize the document: ${document.name}`);
        const result: SummarizeDocumentOutput = await summarizeDocument({ documentDataUri: document.dataUri });
        addMessage('assistant',
          <div>
        ...
        addMessage('user', `Generate a ${action.toUpperCase()} test on "${currentTopic}" ${document ? `using the document ${document.name}` : ''}.`);
        const result = action === "mcq"
          ? await generateMCQTest({ topic: currentTopic, documentDataUri: document?.dataUri, numberOfQuestions: 5 })
          : await generateSubjectiveTest({ topic: currentTopic, documentDataUri: document?.dataUri });
        
        const questions = 'questions' in result ? result.questions : [];
        if (questions.length > 0) {
          const testQuestions = action === 'mcq' 
            ? questions as MCQQuestion[] 
            : (questions as string[]).map(q => ({ question: q }));
          ...
          addMessage('assistant', "Sorry, I couldn't generate a test for that. Please try again.");
        }
      } else if (action === "flashcards") {
        addMessage('user', `Generate flashcards for "${currentTopic}" ${document ? `using the document ${document.name}` : ''}.`);
  ...
  const handlePromptSubmit = async (e: React.FormEvent) => {
    ...
    const topic = currentPrompt.trim();
    const documentToProcess = file;
    
    if (!topic && !documentToProcess && lastTopic) {
       addMessage('user', `Perform ${selectedAction} on "${lastTopic}"`);
       await executeAction(selectedAction, lastTopic);
    } else {
       const promptTopic = topic || (documentToProcess ? documentToProcess.name : '');
       if (!promptTopic) {
       ...
       setCurrentPrompt("");
       setFile(null);
       await executeAction(selectedAction, promptTopic, documentToProcess ?? undefined);
    }
  };
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex h-16 items-center justify-between">
                <Link href="/" className="flex items-center gap-2">
                    ...
                    <span className="font-bold hidden sm:inline-block">UPSC Prep Portal</span>
                </Link>
                <h2 className="text-lg font-semibold text-foreground">
                    Prep Session
                </h2>
             ...
             <div className="space-y-6 pr-4">
              {chatHistory.length === 0 ? (
                <div className="text-center text-muted-foreground pt-16">
                  ...
                  <p>Enter a topic, upload a PDF, and choose an action.</p>
                </div>
              ) : (
                chatHistory.map(msg => (
          ...
          <div className="bg-background">
            <form onSubmit={handlePromptSubmit} className="relative">
              ...
              <div className="flex items-end gap-2 border-t pt-4">
                <Button type="button" variant="outline" size="icon" onClick={() => fileInputRef.current?.click()} title="Upload PDF">
                ...
                <Textarea
                  ...
                  placeholder="Enter a topic, or leave blank to use the last one..."
                  rows={1}
